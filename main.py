...
#artist_name = st.text_input("Please, choose an artist or a group that you would like to search!",
#                            value="Type the name there, please!")

#st.write("So, let's find out if this artist is in the data set: " + artist_name)
...
#st.write(df_of_your_artist["Apple Music"])
...
# df[lambda x: x['artist'] == artist_selection]

# url_blond = df[lambda x: x['artist'] == artist_selection].iloc[:, 10][87]

# st.video(url_blond, format="video", start_time=562)

# st.video(st.write(df[lambda x: x['artist'] == artist_selection].iloc[:, 10][87]),
# format="video/mp4")